...
var rocket_tmpl         = {};
    ...
    rocket_tmpl.header  = function() {
      ...
      return [
               "<script> window.Modernizr || document.write('<script src=\"/js/rocket/vendors/modernizr-2.0.6.custom.min.js\"><\\/script>')</script>"
    ...
    rocket_tmpl.footer  = function() {
      ...
      return [
              ...
              , ( typeof controller_name !== 'undefined' ? "             require(['" + controller_name + "' + '_client' ], function(m) { m.init.apply(m" + (args.length > 0 ? ", " + 'JSON.parse(decodeURIComponent("' + encodeURIComponent(JSON.stringify(args)) + '"))' : '') + ");} ); ": "")
              , "          }"
              , "      }"
              , "  ]);"
              , "</script>"
