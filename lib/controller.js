...
var rocket_tmpl         = {};
    ...
    rocket_tmpl.header  = function() {
      ...
      return [
             ...
             , "<script> window.Modernizr || document.write('<script src=\"/rocket/vendors/modernizr-2.0.6.min.js\"><\\/script>')</script>"
             , (global.require_configuration ? "<script> var require = " + global.require_configuration + "</script>": "")
             , (global.require_configuration ? "<script> require.ready = function() { require(['" + controller_name + "' + '_client' ], function(m) { m.init.apply(m" + (args.length > 0 ? ", " + JSON.stringify(args) : '') + ");} ); }</script>": "")
             ].join('\n');
    };
