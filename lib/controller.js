...
var rocket_tmpl         = {};
    ...
    rocket_tmpl.header  = function() {
      ...
      header.push("<script>")
      header.push("window.Modernizr || document.write('<script src=\"/js/rocket/vendors/modernizr-2.0.6.custom.min.js\"><\\/script>')");
      header.push("var require = " + (global.require_configuration ? global.require_configuration  : "{}" ) + ";");
      
      if (typeof controller_name !== 'undefined') {
        header.push("require.deps || (require.deps = []);");
        header.push("require.deps.unshift('" + controller_name + "_client" + "')");
        header.push("require.callback = function(m) { m && m.init && m.init.apply(m" + (args.length > 0 ? ", " + 'JSON.parse(decodeURIComponent("' + encodeURIComponent(JSON.stringify(args)) + '"))' : '') + "); }");
      }
    ...
    rocket_tmpl.footer  = function() {
      ...
      footer.push("      {");
      footer.push("        load      : '/js/rocket/vendors/require-1.0.1.min.js'");
      footer.push("      , complete  : function() {");
      footer.push("           window.require || Modernizr.load('/js/rocket/vendors/require-1.0.1.min.js')");
      footer.push("        }");
      footer.push("      }");
      
      footer.push("    , { load      : 'preload!/js/" + controller_name + "_client.js' }");
    ...
    if (typeof controller[key] === 'object' && controller[key] !== null) {
      ...
      for (var i = 0, ii = verbs.length; i < ii; i++) {
        ...
        if (['get', 'post', 'put', 'del'].indexOf(current_verb) !== -1) {
          ...
          require.cache[controller_path].exports[key][current_verb] = wrapped;
      ...
      require.cache[controller_path].exports[key] = wrapped;
...
function setupController(app, routes_dadt, controller_file, matches, callback) {
  ...
  controller_info.set('path', controller_path);
