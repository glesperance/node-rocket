...
var rocket_tmpl         = {};
    ...
    rocket_tmpl.header  = function() {
      ...
      boot.push("<script>")
      boot.push("window.Modernizr || document.write('<script src=\"/js/rocket/vendors/modernizr-2.0.6.custom.min.js\"><\\/script>')");
      boot.push("var require = " + (global.require_configuration ? global.require_configuration  : "{}" ) + ";");
      
      /*if (typeof controller_name !== 'undefined') {
        boot.push("require.deps || (require.deps = []);");
        boot.push("require.deps.unshift('" + controller_name + "_client" + "')");
        boot.push("require.callback = function(m) { m.init.apply(m" + (args.length > 0 ? ", " + 'JSON.parse(decodeURIComponent("' + encodeURIComponent(JSON.stringify(args)) + '"))' : '') + "); }");
      }*/
    ...
    rocket_tmpl.footer  = function() {
      ...
      return [
              ...
              , "  Modernizr.load(["
              , "      {"
              , "        load      : '//cdnjs.cloudflare.com/ajax/libs/require.js/1.0.1/require.min.js'"
              , "      , complete  : function() {"
              , "           window.require || Modernizr.load('/js/rocket/vendors/require-1.0.1.min.js')"
              , "        }"
              , "      }"
              , "    , {"
              , "        load      : '/js/" + controller_name + "_client.js'"
              , "      , complete  : function() {
              , "          require('" + controller_name + "_client', function(m) { m.init.apply(m" + (args.length > 0 ? ", " + 'JSON.parse(decodeURIComponent("' + encodeURIComponent(JSON.stringify(args)) + '"))' : '') + "); })"
              , "        }"
              , "      }"
              , "  ]);"
              , "</script>"
