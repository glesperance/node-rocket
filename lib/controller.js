...
var rocket_tmpl         = {};
    ...
    rocket_tmpl.footer  = function() {
      ...
      footer.push("</script>");
      footer.push("<script src=\"//cdnjs.cloudflare.com/ajax/libs/require.js/2.1.1/require.min.js\"></script>");
      footer.push("<script>window.define||document.write('<script src=\"/js/rocket/vendors/require.min.js\"><\\/script>')</script>")
      footer.push("<script src=\"//ajax.googleapis.com/ajax/libs/jquery/1.8.3/jquery.min.js\"></script>")

      if (process.env['NODE_ENV'] === 'production')
        footer.push("<script src=\"/js/" + controller_name + "_client.js?" + require_configuration.urlArgs + "\"></script>")
...
function buildWrapper(controller_info, method_func) {
  ...
  return function(req, res, next) {
    ...
    var view_file   = controller_info.get('view')
      ...
      , __          = !layout_node && controller_info.parent.parent && (layout_node = controller_info.parent.parent.get('layout'))
    ...
    res._boot_options || (res._boot_options = {});
    ...
    if (!req.xhr && view_file) {
      ...
      res.send = function(obj) {
        ...
        obj.boot_options = _.extend(res._boot_options, obj.boot_options);
