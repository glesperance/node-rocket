...
exports.setup = function setup_client() {
   ...
   * as `/static`
   */
  app.use('/static', express.static(static_dir_path, { maxAge: oneDay }));
  
  /***
   * sets up a static middleware to serve the rocket `rocket-js` folder
   * as `/rocket-js`
   */
  app.use('/rocket-js', express.static(path.join(__dirname, 'rocket-js'), { maxAge: oneDay }));
  ...
  if (process.env['NODE_ENV'] === 'production') {
    ...
    async.parallel([
        function(callback) { async.waterfall([
          ...
          , function(callback) {
              ...
              //as `/js`
              app.use('/js', express.static(client_js_tmpdir.path, { maxAge: oneDay }));
      ...
      , function(callback) { async.waterfall([
          ...
          , function(callback) {
              ...
              //as `/css`
              app.use('/css', express.static(client_css_tmpdir.path, { maxAge: oneDay }));
    ...
    //as `/js`
    app.use('/js', express.static(js_dir_path, { maxAge: oneDay }));
    app.use('/css', express.static(css_dir_path, { maxAge: oneDay }));
