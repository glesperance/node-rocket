...
# Thanks to NPM for their code!

echo "This script is not yet finished. Thanks."
exit 1

if [ "x$0" = "xsh" ]; then
  # run as curl | sh
  # on some systems, you can just do cat>rocket-install.sh
  # which is a bit cuter.  But on others, &1 is already closed,
  # so catting to another script file won't do anything.
  curl -s http://rocketjs.org/install.sh > rocket-install-$$.sh
  sh rocket-install-$$.sh
  ret=$?
  rm rocket-install-$$.sh
  exit $ret
fi

if ! [ "x$rocket_DEBUG" = "x" ]; then
  set +x
fi
...
if [ $ret -ne 0 ] || ! [ -x $node ]; then
  echo "rocket cannot be installed without nodejs." >&2
...
url=`curl -s -L http://registry.rocketjs.org/rocket/$t \
      | $egrep -o 'tarball":"[^"]+' \
      | head -n 1 \
      | $egrep -o 'http://.*'`
echo "fetching: $url" >&2
...
cd "$TMP" \
  ...
  && (ver=`$node bin/read-package-json.js package.json version`
      ...
      ret=0
      if [ $isrocket10 -eq 1 ] && [ -f "scripts/clean-old.sh" ]; then
        if ! [ "x$skipclean" = "x" ] \
            || [ "x$clean" = "xno" ] \
            || [ "x$clean" = "xn" ]; then
  ...
  && (if [ "$make" = "NOMAKE" ] || ! $make clean install; then
        ...
        $node cli.js rm rocket --force --global
